...
# 내풀이
# 규칙 -> 첫번째 나오는 숫자와 달라지는 시점 횟수만큼 뒤집어야 함
def find_count_to_turn_out_to_all_zero_or_all_one(string):
    first_num = string[0]
    sum = 0
    for num in range(1, len(string)):
        if string[num] != first_num and string[num] != string[num-1]: # 첫번째 숫자와 다르고 직전 숫자와 달라지는 시점에만 뒤집어야 하는 횟수 추가
            sum += 1
    return sum
